...
def get_gdrive_service(scopes=None):
    ...
    gdrive_service = build(
        'drive', 'v3', http=creds.authorize(Http()), cache_discovery=False)
    return gdrive_service
...
def gdrive_find_folder(drive_service, foldername):
    files = gdrive_find_files(drive_service,
                              "mimeType='application/vnd.google-apps.folder' and name='{}'".format(foldername))
    if len(files) > 1:
        raise RuntimeError('Found more than one folder named {}, I can not handle this situation')
    elif len(files) == 1:
        return files[0]
    else:
        return None
...
def gdrive_create_folder(drive_service, foldername, parents=None):
    ...
    file = drive_service.files().create(body=file_metadata,
                                        fields='id').execute()
    logger.info('folder {name} id: {file_id}'.format(name=foldername, file_id=file.get('id')))
    return file
...
if __name__ == '__main__':
    ...
    gdrive_create_file(drive_service, 'combined_pdfs/22-07-2018_23.11.50.pdf', '22-07-2018_23.11.50.pdf', scans_folder)
